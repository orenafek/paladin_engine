...
class Block(Rectangle):
    ...
    OUTLINE_WIDTH = 3
    __AS__(('OUTLINE_WIDTH', OUTLINE_WIDTH), locals=locals(), globals=
        globals(), frame=sys._getframe(0))

    def __init__(self, pos, color):
        self.x = pos.x
        __AS__(('self', self), locals=locals(), globals=globals(), frame=
            sys._getframe(0))
        self.y = pos.y
        __AS__(('self', self), locals=locals(), globals=globals(), frame=
            sys._getframe(0))
        ...
        __AS__(('p1', p1), locals=locals(), globals=globals(), frame=sys.
            _getframe(0))
        p2 = Point(p1.x + Block.BLOCK_SIZE, p1.y + Block.BLOCK_SIZE)
        __AS__(('p2', p2), locals=locals(), globals=globals(), frame=sys.
            _getframe(0))
    ...
    def new_pos_after_rotate(self, dir, center):
        ...
        __AS__(('rot_x', rot_x), locals=locals(), globals=globals(), frame=
            sys._getframe(0))
        rot_y = center.y + dir * center.x - dir * self.x
        __AS__(('rot_y', rot_y), locals=locals(), globals=globals(), frame=
            sys._getframe(0))
        return rot_x, rot_y
...
class Shape(object):
    ...
    def can_rotate(self, board):
        ...
        __AS__(('rotation_direction', rotation_direction), locals=locals(),
            globals=globals(), frame=sys._getframe(0))
        for block in self.blocks:
...
class Board(object):
    ...
    def draw_shape(self, shape):
        ...
        if self.__DEBUG_X != -1 and self.__DEBUG_Y != -1:
            ...
            __AS__(('self', self), ('self', self), ('self', self), locals=
                locals(), globals=globals(), frame=sys._getframe(0))
    ...
    def delete_row(self, y):
        ...
        for _x, _y in filter(lambda k: k[1] == y, self.grid.copy().keys()):
            ...
            __AS__(('self', self), locals=locals(), globals=globals(),
                frame=sys._getframe(0))
        __POST_CONDITION__(locals=locals(), globals=globals(), frame=sys._getframe(0))
...
class Tetris(object):
    ...
    __AS__(('SHAPES', SHAPES), locals=locals(), globals=globals(), frame=
        sys._getframe(0))
    ...
    __AS__(('BOARD_HEIGHT', BOARD_HEIGHT), locals=locals(), globals=globals
        (), frame=sys._getframe(0))
    __TEST_MODE = True
    __AS__(('__TEST_MODE', __TEST_MODE), locals=locals(), globals=globals(),
        frame=sys._getframe(0))
    ...
    def key_pressed(self, event):
        ...
        """
        key = event.keysym
        __AS__(('key', key), locals=locals(), globals=globals(), frame=sys.
            _getframe(0))
...
if __name__ == '__main__':
    win = Window('Tetris')
    __AS__(('win', win), locals=locals(), globals=globals(), frame=sys.
        _getframe(0))
    game = Tetris(win)
    __AS__(('game', game), locals=locals(), globals=globals(), frame=sys.
        _getframe(0))
    win.mainloop()
