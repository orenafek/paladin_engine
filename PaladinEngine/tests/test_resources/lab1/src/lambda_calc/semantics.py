...
class LambdaInterpreter(object):
    ...
    def normal_form(self, expr):
        ...
        while True:
            ...
            if t: print(" ->", pretty(expr.subtrees[0]))

        if not t: print(" ->", pretty(expr.subtrees[0]))
...
if __name__ == '__main__':
    ...
    p.add_argument('n', metavar='N', type=int, nargs='?', default=3,
                   help="(demo) argument to pass to `fact`")
    p.add_argument('--no-trace', action='store_true', default=False,
                   help="disable printing of the derivation; only normal form")
    o = p.parse_args()
    ...
    if expr:
        ...
        print(expr)
        LambdaInterpreter(
            LambdaInterpreter.Options(trace=False))(expr)
    else:
